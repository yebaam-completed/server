...
class NotificationWorker {
  async updateNotification(job: Job, done: DoneCallback): Promise<void> {
    try {
      const { key } = job.data;
      log.info(`Updating notification with key: ${key}`); // Log de depuración
      await notificationService.updateNotification(key);
      job.progress(100);
      done(null, job.data);
      log.info(`Notification updated successfully for key: ${key}`); // Log de depuración
    } catch (error) {
      log.error(error);
      done(error as Error);
    }
  ...
  async deleteNotification(job: Job, done: DoneCallback): Promise<void> {
    try {
      const { key } = job.data;
      log.info(`Deleting notification with key: ${key}`); // Log de depuración

      await notificationService.deleteNotification(key);
      job.progress(100);
      done(null, job.data);
      log.info(`Notification deleted successfully for key: ${key}`); // Log de depuración
    } catch (error) {
      log.error(error);
      done(error as Error);
    }
